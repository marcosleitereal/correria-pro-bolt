...
Deno.serve(async (req) => {
  try {
    ...
    if (settingsError) {
      ...
      console.error('❌ Query executada: SELECT * FROM app_settings ORDER BY updated_at DESC LIMIT 1');
      return corsResponse({ 
        error: 'ERRO CRÍTICO: Configurações de trial não encontradas no Painel Admin. Verifique se a tabela app_settings possui dados ou configure primeiro no Admin Dashboard.' 
      }, 500);
    }

    if (!appSettings) {
      console.error('❌ CREATE-USER: Nenhuma configuração encontrada na tabela app_settings');
      console.error('❌ A tabela app_settings está vazia ou a query não retornou resultados');
      return corsResponse({ 
        error: 'ERRO CRÍTICO: Configurações de trial não configuradas no Painel Admin. A tabela app_settings está vazia. Configure primeiro no Admin Dashboard.' 
      }, 500);
    }
    ...
    console.log('✅ CREATE-USER: Configurações do Painel Admin carregadas (VALORES REAIS):', {
      ...
      observacao: 'Estes devem ser os valores configurados no Admin Panel: 25 dias, 30 atletas, 12 treinos'
    });
